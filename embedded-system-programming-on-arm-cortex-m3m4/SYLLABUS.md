...
- [x] **11. Bare-metal build with GNU tools** (277–344) — cross-compilation without an
  ...
  *Done as `gnu-build/`: a self-contained folder with its own hand Makefile, `startup.c`,
  ...
  ST's startup/linker script stay in the CMake project untouched. Board run pending SWD.*
- [ ] **12. OpenOCD, newlib & semihosting** (345–378) — downloading/debugging with
  ...
  *Toolchain note: OpenOCD 0.12 (ST fork) is bundled with CubeIDE 2.0 and already wired
  into `gnu-build/Makefile` (`make openocd` / `make load`); `ST-LINK_gdbserver` also has a
  `--semihosting` option for the semihosting lesson.*
